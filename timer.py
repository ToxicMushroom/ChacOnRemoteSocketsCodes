...
def mainloop(sc):
    ...
    for row1 in result:  # loop thru all results
        if row1[2] == 1:  # If trigger enabled continue
            cursor.execute("SELECT * FROM chacon_times WHERE triggerId= " + str(row1[0]))  # execute more info query
            ...
            for row2 in result:  # Lets go through all enabled rows in chacon_times to check if the time for execution
                ...
                if row2[2] == "sunrise":  # Replace the sunrise placeholder with the current sunrise time
                    if str(weekday) in row2[1] and seconds_since_midnight + 30 > sunrise > seconds_since_midnight - 30:
                        for code in re.split(", ", row1[1]):
                            rfdevice.tx_code(int(code), 1, 430)
                elif row2[2] == "sunset":  # same here
                    if str(weekday) in row2[1] and seconds_since_midnight + 30 > sunset > seconds_since_midnight - 30:
                        for code in re.split(", ", row1[1]):
                            rfdevice.tx_code(int(code), 1, 430)
                else:
                    if str(weekday) in row2[1] and seconds_since_midnight + 30 > int(
                            row2[2]) > seconds_since_midnight - 30:
                        for code in re.split(", ", row1[1]):
